...
import { updateAnnonceById } from "../../redux/Actions/annonceActions";

export default function EditPropertyDetailsScreen({ navigation, route }) {
    const {
        annonce,
        description,
        images,
    ...
    const [etage, setEtage] = useState(annonce.property.etage?.toString() || "");
    const [avecAscenceur, setAscenceur] = useState(annonce.property.avecAscenceur || false);
    const [nbEtages, setNbEtages] = useState(annonce.property.nbEtages?.toString() || "");
    const [avecGarage, setAvecGarage] = useState(annonce.property.avecGarage || false);
    const [avecSousSol, setAvecSousSol] = useState(annonce.property.avecSousSol || false);
    const [surfaceJardin, setSurfaceJardin] = useState(annonce.property.surfaceJardin?.toString() || "");
    const [avecPiscine, setAvecPiscine] = useState(annonce.property.avecPiscine || false);
    ...
    const handleSubmit = async () => {
        ...
        try {
          const response = await dispatch(updateAnnonceById(annonce.id, formData));
          if (response.success) {
            navigation.navigate("Home");
          } else {
            console.error("Failed to update annonce:", response.error);
          }
        } catch (error) {
    ...
    return (
        <ScrollView contentContainerStyle={styles.container}>
            <Text style={styles.title}>Modification d'une Annonce</Text>
            ...
            <ButtonC title="Modifier l'Annonce" color="#4CAF50" onPress={handleSubmit} />
        </ScrollView>
    );
}
