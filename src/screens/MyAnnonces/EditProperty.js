...
export default function EditPropertyScreen({ navigation, route }) {
    const { annonce, description, images } = route.params;
    const [adresse, setAdresse] = useState(annonce.property.adresse || "");
    const [ville, setVille] = useState(annonce.property.ville || "");
    const [superficie, setSuperficie] = useState(annonce.property.superficie?.toString() || "");
    const [prix, setPrix] = useState(annonce.property.prix?.toString() || "");
    const [type, setType] = useState(annonce.property.type || "Villa");
    const [status, setStatus] = useState(annonce.property.status || "A vendre");
    const [nbChambres, setNbChambres] = useState(annonce.property.nbChambres?.toString() || "");
    const [nbSallesDeBain, setNbSallesDeBain] = useState(annonce.property.nbSallesDeBain?.toString() || "");
    const [mesureSon, setMesureSon] = useState(annonce.property.mesureSon?.toString() || "");
    const [mesureCO2, setMesureCO2] = useState(annonce.property.mesureCO2?.toString() || "");
